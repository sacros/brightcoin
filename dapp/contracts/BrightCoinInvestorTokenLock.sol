...
enum BrightCoinLockType { Investor, Admin,Bounty }
   ...
   constructor() public 
   {
    ...
    function isTokenLockExpire(address _of, uint256 _time)  view internal  returns(bool)
    {
       ...
       {
          //emit Unlocked(_of, "Token lock expired",locktokenDetails[_of].amount);
          return retVal = true;
       }
    ...
    //Increase token lock validity
    function IncreaseTokenValidity(address _addr, uint256 _validity)
         internal 
        returns (bool)
    {
       
        require(tokensLocked(_addr) > 0); //The token must be there always
         lockToken storage AddrStruct =  locktokenDetails[_addr];
        AddrStruct.validity = _validity;
        emit LockUpdated(msg.sender, _addr, _validity, "Token lock Updated");
        return true;
